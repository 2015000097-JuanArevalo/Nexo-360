...
3. El personal técnico abre la bandeja en tiempo real y aprueba o deniega. Al aprobar, se crea el permiso real de forma atómica.
4. El estudiante ve sus permisos en tiempo real y presenta un QR real.
5. Docentes y personal técnico validan el QR con cámara o con el código manual.
6. La pantalla de resultado distingue: válido, expirado, no autorizado, no encontrado, inválido y aún no vigente.
...
| `app/lib/features/permisos/pending_permission_requests_screen.dart` | Bandeja técnica de aprobación y denegación |
...
| Cuenta | Acción |
| --- | --- |
| `technical` | Selecciona estudiantes, crea permisos, procesa solicitudes y valida QR |
| `teacher` | Selecciona estudiantes, crea `permission_requests` pendientes y valida QR |
| Organizador | Crea `permission_requests` pendientes |
| `student` | Consulta únicamente sus permisos y presenta su QR |

La ruta `/permissions/validate` está disponible para `technical` y `teacher`. La ruta `/permissions/requests` está limitada a `technical`. La protección de Flutter mejora la experiencia, pero la protección real depende de las reglas de Firestore.
